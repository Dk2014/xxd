...
func (req *IDIP_DO_SET_SOUL_LEVEL_REQ) Process() (IDIP_DO_SET_SOUL_LEVEL_RSP, error) {
	st := &platform_server.ServerType{AreaId: uint8(req.Body.AreaId), PlatId: req.Body.PlatId}
	rsp := IDIP_DO_SET_SOUL_LEVEL_RSP{}
	...
	if exist {
		rpc.RemoteIdipSetSoulLevel(req.Body.OpenId, int(gs.GSId), req.Body.SoulId, req.Body.Value, func(Reply *rpc.Reply_IdipSetSoulLevel, err error) {
			if err != nil {
				rsp.Body.Result = 2
