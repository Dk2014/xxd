...
func (req *IDIP_DO_UPDATE_ROLE_COIN_REQ) Process() (IDIP_DO_UPDATE_ROLE_COIN_RSP, error) {
	st := &platform_server.ServerType{AreaId: uint8(req.Body.AreaId), PlatId: req.Body.PlatId}
	rsp := IDIP_DO_UPDATE_ROLE_COIN_RSP{}
	...
	if exist {
		rpc.RemoteIdipUpdateMoney(req.Body.OpenId, int(gs.GSId), int64(req.Body.RoleId), int64(req.Body.Value), player_dat.COINS, func(Reply *rpc.Reply_IdipUpdateMoney, err error) {
			if err != nil {
				rsp.Body.Result = 2
