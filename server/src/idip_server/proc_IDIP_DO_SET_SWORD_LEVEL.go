...
func (req *IDIP_DO_SET_SWORD_LEVEL_REQ) Process() (IDIP_DO_SET_SWORD_LEVEL_RSP, error) {
	st := &platform_server.ServerType{AreaId: uint8(req.Body.AreaId), PlatId: req.Body.PlatId}
	rsp := IDIP_DO_SET_SWORD_LEVEL_RSP{}
	...
	if exist {
		rpc.RemoteIdipSetSwordLevel(req.Body.OpenId, int(gs.GSId), req.Body.RoleId, req.Body.Pos, req.Body.Value, func(Reply *rpc.Reply_IdipSetSwordLevel, err error) {
			if err != nil {
				rsp.Body.Result = 2
