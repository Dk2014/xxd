...
func (req *IDIP_DO_UPDATE_ROLE_EXP_REQ) Process() (IDIP_DO_UPDATE_ROLE_EXP_RSP, error) {
	st := &platform_server.ServerType{AreaId: uint8(req.Body.AreaId), PlatId: req.Body.PlatId}
	rsp := IDIP_DO_UPDATE_ROLE_EXP_RSP{}
	...
	if exist {
		rpc.RemoteIdipUpdateExp(req.Body.OpenId, int(gs.GSId), int64(req.Body.RoleId), int64(req.Body.Value), func(Reply *rpc.Reply_IdipUpdateExp, err error) {
			if err != nil {
				rsp.Body.Result = 2
