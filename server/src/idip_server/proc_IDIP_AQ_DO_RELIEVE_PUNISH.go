...
func (req *IDIP_AQ_DO_RELIEVE_PUNISH_REQ) Process() (IDIP_AQ_DO_RELIEVE_PUNISH_RSP, error) {
	st := &platform_server.ServerType{AreaId: uint8(req.Body.AreaId), PlatId: req.Body.PlatId}
	rsp := IDIP_AQ_DO_RELIEVE_PUNISH_RSP{}
	...
	if exist {
		rpc.RemoteIdipAqRelievePunish(req.Body.OpenId, int(gs.GSId), uint8(req.Body.RelieveBan), func(Reply *rpc.Reply_IdipAqRelievePunish, err error) {
			if err != nil {
				rsp.Body.Result = 2
