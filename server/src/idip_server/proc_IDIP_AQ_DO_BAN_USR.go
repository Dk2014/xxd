...
func (req *IDIP_AQ_DO_BAN_USR_REQ) Process() (IDIP_AQ_DO_BAN_USR_RSP, error) {
	st := &platform_server.ServerType{AreaId: uint8(req.Body.AreaId), PlatId: req.Body.PlatId}
	rsp := IDIP_AQ_DO_BAN_USR_RSP{}
	...
	if exist {
		rpc.RemoteIdipAqBanUser(req.Body.OpenId, int(gs.GSId), uint32(req.Body.Time), func(Reply *rpc.Reply_IdipAqBanUser, err error) {
			if err != nil {
				rsp.Body.Result = 2
