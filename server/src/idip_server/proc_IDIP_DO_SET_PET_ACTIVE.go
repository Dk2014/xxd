...
func (req *IDIP_DO_SET_PET_ACTIVE_REQ) Process() (IDIP_DO_SET_PET_ACTIVE_RSP, error) {
	st := &platform_server.ServerType{AreaId: uint8(req.Body.AreaId), PlatId: req.Body.PlatId}
	rsp := IDIP_DO_SET_PET_ACTIVE_RSP{}
	...
	if exist {
		rpc.RemoteIdipAddPet(req.Body.OpenId, int(gs.GSId), int16(req.Body.PetId), func(Reply *rpc.Reply_IdipAddPet, err error) {
			if err != nil {
				rsp.Body.Result = 2
