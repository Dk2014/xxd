...
func (req *IDIP_DO_DEL_ITEM_REQ) Process() (IDIP_DO_DEL_ITEM_RSP, error) {
	st := &platform_server.ServerType{AreaId: uint8(req.Body.AreaId), PlatId: req.Body.PlatId}
	rsp := IDIP_DO_DEL_ITEM_RSP{}
	...
	if exist {
		rpc.RemoteIdipDelItem(req.Body.OpenId, int(gs.GSId), int16(req.Body.ItemId), int16(req.Body.ItemNum), func(Reply *rpc.Reply_IdipDelItem, err error) {
			if err != nil {
				rsp.Body.Result = 2
