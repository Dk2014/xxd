...
func (req *IDIP_DO_SEND_MAIL_ALL_REQ) Process() (IDIP_DO_SEND_MAIL_ALL_RSP, error) {
	st := &platform_server.ServerType{AreaId: uint8(req.Body.AreaId), PlatId: req.Body.PlatId}
	rsp := IDIP_DO_SEND_MAIL_ALL_RSP{}
	...
	if req.Body.Partition == 0 {
		...
		for _, v := range server_list {
			if v.Type == st.GetType() {
				for _, v2 := range v.GServers {
					if v2.HD == true {
						hasHd = true
						rpc.RemoteIdipSendAllMail(int(v2.GSId), mailtitle, mailcontent, req.Body.SendTime, req.Body.EndTime, itemdetail, func(Reply *rpc.Reply_IdipSendMail, err error) {
							if err != nil {
								rsp.Body.Result = 2
		...
		if exist {
			...
			for _, v := range gs.GServers {
				if v.HD == true {
					hasHd = true
					rpc.RemoteIdipSendAllMail(int(v.GSId), mailtitle, mailcontent, req.Body.SendTime, req.Body.EndTime, itemdetail, func(Reply *rpc.Reply_IdipSendMail, err error) {
						if err != nil {
							rsp.Body.Result = 2
							rsp.Body.RetMsg = fmt.Sprintf("%v", err)
							rsp.Head.Result = -101
