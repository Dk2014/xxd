...
/*
	解冻帐号
*/
func (req *IDIP_DO_UNBAN_USR_REQ) Process() (IDIP_DO_UNBAN_USR_RSP, error) {
	st := &platform_server.ServerType{AreaId: uint8(req.Body.AreaId), PlatId: req.Body.PlatId}
	rsp := IDIP_DO_UNBAN_USR_RSP{}
	...
	if exist {
		rpc.RemoteIdipUnBanUser(req.Body.OpenId, int(gs.GSId), func(Reply *rpc.Reply_IdipUnBanUser, err error) {
			if err != nil {
				rsp.Body.Result = 2
